  ...
  return (
    <div className="mb-8 font-plex text-[12px] reveal">
      <div className="mb-3 flex items-center gap-2">
        <span className="text-accent uppercase tracking-[0.2em] text-[11px]">// deployment summary</span>
  ...
  if (status === "SUCCESS") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded border px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest"
        ...
        <span className="status-dot h-1.5 w-1.5 rounded-full" style={{ background: "var(--success)" }} />
        SUCCESS
      </span>
    );
  }
  ...
  return (
    <div className="mt-8 panel rounded-md p-4 font-mono text-[12px] reveal">
      <div className="flex items-center gap-2 border-b border-border pb-2 mb-3">
        ...
        <span className="h-2 w-2 rounded-full bg-muted-foreground/40" />
        <span className="h-2 w-2 rounded-full bg-muted-foreground/40" />
        <span className="ml-2 text-[10px] uppercase tracking-widest text-muted-foreground">deploy.sh</span>
      </div>
      <div className="text-muted-foreground">
        <div><span className="text-accent">$</span> deployment status</div>
        {PROJECTS.map((p) => (
          <div key={p.id} className="text-muted-foreground">
            ...
            <span className={p.status === "SUCCESS" ? "text-success" : "text-muted-foreground/70"}>{p.status}</span>
          </div>
        ))}
        <div className="mt-3 pt-3 border-t border-border grid gap-1 sm:grid-cols-2">
          <div>Pipeline Status: <span className="text-success">Healthy</span></div>
          <div>Latest Deployment: <span className="text-foreground">DEP-2026-003</span></div>
        </div>
      </div>
    </div>
  );
